...
# ─── MODÜL 5: Oyuncu İstekleri (Wish Analizi) ─────────────────────────────────

_WISH_PAT_EN = re.compile(
    r'\b(wish(?:es|ed|ing)?|hope[sd]?|would be better|should have|'
    r'needs?\s+to|missing|lacks?|lack(?:ing)?|'
    r'could use|if only|please add|want(?:ed|ing|s)?|would love|'
    r'should add|would like|needs? more|bring back|'
    r'they should|devs? should|i\'d like|i wish|i hope|'
    r'could be better|needs improvement|should be|add more|'
    r'more (?:content|options|features|maps|modes))\b',
    re.IGNORECASE,
)
_WISH_PAT_TR = re.compile(
    r'(keşke|olsaydı|olsa iyi olurdu|eksik(?:lik)?|olmalı|olmalıydı|'
    r'istiyorum|ekleyin|olsa iyi olur|bekliyorum|lazım|gerekli|'
    r'eklenmeli|olmasını|umuyorum|dilerim|eklensin|olurdu)',
    re.IGNORECASE,
)
_WISH_KEYWORDS = [
    "wish", "hope", "missing", "lack", "need", "needs", "want", "wants",
    "add", "should", "could", "would", "better", "more", "please",
    "keşke", "eksik", "olmalı", "istiyorum", "ekleyin", "lazım",
    "eklenmeli", "umuyorum",
]


def _istek_bul(df: pd.DataFrame) -> pd.DataFrame:
    pos = df[df["voted_up"] == True].copy() if "voted_up" in df.columns else df.copy()
    text_col = next((c for c in ("review_english", "review_clean", "review") if c in pos.columns), None)
    if text_col is None or pos.empty:
        return pd.DataFrame()
    metin_ser = pos[text_col].fillna("").astype(str)
    mask = metin_ser.str.contains(_WISH_PAT_EN) | metin_ser.str.contains(_WISH_PAT_TR)
    eslesme = pos[mask].copy()
    eslesme["_text_col"] = text_col
    return eslesme


def _wish_frekans(df: pd.DataFrame) -> pd.Series:
    text_col = next((c for c in ("review_english", "review_clean", "review") if c in df.columns), None)
    if text_col is None or df.empty:
        return pd.Series(dtype=int)
    metin = " ".join(df[text_col].fillna("").astype(str)).lower()
    kelimeler = re.findall(r"\b[a-zA-ZğüşöçıİĞÜŞÖÇ]{2,}\b", metin)
    sayimlar = {kw: kelimeler.count(kw.lower()) for kw in _WISH_KEYWORDS}
    return pd.Series({k: v for k, v in sayimlar.items() if v > 0}).sort_values(ascending=False)


def render_oyuncu_istekleri(lang: str):
    if lang == "TR":
        st.markdown("## 💡 Oyuncu İstekleri")
        st.caption("Olumlu yorumlardan gizli istek ve öneri cümlelerini tespit edin")
        oyun_lbl   = "Oyun Seçin"
        sol_baslik = "🗂️ Tespit Edilen İstek Yorumları"
        sag_baslik = "📊 En Sık İstek Kelimeleri"
        ozet_lbl   = "📋 Özet"
        pt_lbl     = "Oyun süresi"
        saat_lbl   = "saat"
        no_data    = "Yeterli veri bulunamadı."
        no_wish    = "Bu oyun için istek içeren yorum tespit edilemedi."
    else:
        st.markdown("## 💡 Player Wish Analysis")
        st.caption("Detect hidden wish and request sentences from positive reviews")
        oyun_lbl   = "Select Game"
        sol_baslik = "🗂️ Detected Wish Reviews"
        sag_baslik = "📊 Most Common Wish Keywords"
        ozet_lbl   = "📋 Summary"
        pt_lbl     = "Playtime"
        saat_lbl   = "hrs"
        no_data    = "Not enough data available."
        no_wish    = "No wish reviews found for this game."

    tum_oyunlar = [(o, OYUN_ADLARI.get(o, o)) for tur in TURLER.values() for o in tur]
    oyun_idx    = st.selectbox(
        oyun_lbl, range(len(tum_oyunlar)),
        format_func=lambda i: tum_oyunlar[i][1], key="istek_oyun",
    )
    secili_oyun = tum_oyunlar[oyun_idx][0]
    ...
    try:
        ...
        eslesme    = _istek_bul(df)
        toplam_pos = int(df["voted_up"].astype(bool).sum()) if "voted_up" in df.columns else len(df)

        if eslesme.empty:
            st.info(no_wish)
            return

        istek_pct = len(eslesme) / max(1, toplam_pos) * 100

        sol, sag = st.columns([1.1, 0.9])

        with sol:
            st.markdown(
                f"<div class='sec-title'>{sol_baslik} ({len(eslesme):,})</div>",
                unsafe_allow_html=True,
            )
            text_col = (eslesme["_text_col"].iloc[0]
                        if "_text_col" in eslesme.columns else "review")
            goster = eslesme.head(15)
            for _, row in goster.iterrows():
                ham   = str(row.get(text_col, ""))
                metin = (ham[:300] + "…") if len(ham) > 300 else ham
                pt    = row.get("playtime_forever", 0)
                try:
                    pt_saat = round(int(pt) / 60, 1)
                except Exception:
                    pt_saat = 0
                dil = str(row.get("detected_language", row.get("language", "?")))[:10]
                st.markdown(
                    f'<div style="background:#1e293b;border-left:3px solid #3b82f6;'
                    f'border-radius:6px;padding:0.6rem 0.8rem;margin-bottom:0.5rem;">'
                    f'<div style="font-size:0.82rem;color:#cbd5e1;line-height:1.55;">{metin}</div>'
                    f'<div style="font-size:0.72rem;color:#64748b;margin-top:0.3rem;">'
                    f'{pt_lbl}: {pt_saat} {saat_lbl} · {dil}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )

        with sag:
            st.markdown(f"<div class='sec-title'>{sag_baslik}</div>", unsafe_allow_html=True)
            frekans = _wish_frekans(eslesme)
            if not frekans.empty:
                top_f = frekans.head(14)
                freq_lbl = "Geçme Sayısı" if lang == "TR" else "Frequency"
                kw_lbl   = "Anahtar Kelime" if lang == "TR" else "Keyword"
                fig_w = go.Figure(go.Bar(
                    ...
                    textposition="outside",
                    hovertemplate=(
                        f"<b>%{{y}}</b><br>{freq_lbl}: %{{x}}<extra></extra>"
                    ),
                ))
                fig_w.update_layout(
                    template="plotly_dark", height=370,
                    margin=dict(t=10, b=10, l=10, r=50),
                    xaxis_title=freq_lbl,
                    yaxis=dict(autorange="reversed", title=kw_lbl),
                )
                st.plotly_chart(fig_w, use_container_width=True)

            st.markdown(f"<div class='sec-title'>{ozet_lbl}</div>", unsafe_allow_html=True)
            en_sik     = frekans.index[0] if not frekans.empty else "-"
            en_sik_say = int(frekans.iloc[0]) if not frekans.empty else 0
            oyun_ad    = OYUN_ADLARI.get(secili_oyun, secili_oyun)
            if lang == "TR":
                st.metric("İstek Yorumu Oranı",    f"%{istek_pct:.1f}",
                          f"{len(eslesme):,} / {toplam_pos:,} olumlu yorum")
                st.metric("En Sık İstek Kelimesi", en_sik, f"{en_sik_say}x")
                st.info(
                    f"**{oyun_ad}**'in olumlu yorumlarının **%{istek_pct:.1f}**'i "
                    f"gizli istek içeriyor. "
                    f"En çok istenen özellik/kelime: **{en_sik}**. "
                    f"Toplam **{len(eslesme):,}** istek yorumu tespit edildi."
                )
            else:
                st.metric("Wish Review Rate",        f"{istek_pct:.1f}%",
                          f"{len(eslesme):,} / {toplam_pos:,} positive reviews")
                st.metric("Most Common Wish Word", en_sik, f"{en_sik_say}x")
                st.info(
                    f"**{istek_pct:.1f}%** of **{oyun_ad}**'s positive reviews "
                    f"contain hidden wishes. "
                    f"Most requested feature/keyword: **{en_sik}**. "
                    f"Total **{len(eslesme):,}** wish reviews detected."
                )
    except Exception:
        st.warning(no_data)

    st.divider()
    _render_pdf_butonu(secili_oyun, lang, "istek")


# ─── MODÜL 6: Dil Bazlı Şikayet Farklılığı ───────────────────────────────────

def _dil_metrikleri(df: pd.DataFrame) -> pd.DataFrame:
    dil_col = next((c for c in ("detected_language", "language") if c in df.columns), None)
    if dil_col is None:
        return pd.DataFrame()
    kat_keys   = list(_KAT_TR.keys())
    dil_sayilari = df[dil_col].value_counts()
    top_diller   = dil_sayilari[dil_sayilari >= 10].head(8).index.tolist()
    satirlar = []
    for dil in top_diller:
        alt    = df[df[dil_col] == dil]
        toplam = len(alt)
        if toplam < 5:
            continue
        pos_pct = (float(alt["voted_up"].astype(bool).mean() * 100)
                   if "voted_up" in alt.columns else 0.0)
        satir = {
            "dil_kod": dil,
            "dil_ad":  _DIL_GORUNUM.get(str(dil).lower(), str(dil).capitalize()),
            "toplam":  toplam,
            "pos_pct": pos_pct,
            "neg_pct": 100.0 - pos_pct,
        }
        for k in kat_keys:
            satir[k] = float(alt[k].mean() * 100) if k in alt.columns else 0.0
        satirlar.append(satir)
    return pd.DataFrame(satirlar)


def _grafik_dil_sikayet(dil_df: pd.DataFrame, lang: str) -> go.Figure:
    etiketler = kat_etiketler(lang)
    kat_keys  = [k for k in _KAT_TR.keys() if k in dil_df.columns]
    renkler   = ["#3b82f6", "#ef4444", "#22c55e", "#f59e0b",
                 "#8b5cf6", "#ec4899", "#14b8a6", "#f97316"]
    title = ("Dil Bazlı Şikayet Dağılımı (%)" if lang == "TR"
             else "Complaint Distribution by Language (%)")
    fig = go.Figure()
    for i, k in enumerate(kat_keys):
        kat_et = etiketler[k]
        fig.add_trace(go.Bar(
            ...
            hovertemplate=f"<b>%{{x}}</b><br>{kat_et}: %{{y:.1f}}%<extra></extra>",
        ))
    fig.update_layout(
        barmode="group", title=title, template="plotly_dark",
        height=390, margin=dict(t=50, b=140, l=0, r=0),
        legend=dict(orientation="h", yanchor="bottom", y=-0.55, xanchor="center", x=0.5),
        xaxis_title="Dil" if lang == "TR" else "Language",
        yaxis_title="Şikayet Oranı (%)" if lang == "TR" else "Complaint Rate (%)",
    )
    return fig


def _grafik_dil_duygu(dil_df: pd.DataFrame, lang: str) -> go.Figure:
    title   = ("Dil Bazlı Duygu Karşılaştırması" if lang == "TR"
               else "Sentiment Comparison by Language")
    pos_lbl = "Olumlu %" if lang == "TR" else "Positive %"
    neg_lbl = "Olumsuz %" if lang == "TR" else "Negative %"
    fig = go.Figure()
    fig.add_trace(go.Bar(
        name=pos_lbl, x=dil_df["pos_pct"].round(1), y=dil_df["dil_ad"],
        orientation="h", marker_color="#22c55e",
        hovertemplate=f"<b>%{{y}}</b><br>{pos_lbl}: %{{x:.1f}}%<extra></extra>",
    ))
    fig.add_trace(go.Bar(
        name=neg_lbl, x=dil_df["neg_pct"].round(1), y=dil_df["dil_ad"],
        orientation="h", marker_color="#ef4444",
        hovertemplate=f"<b>%{{y}}</b><br>{neg_lbl}: %{{x:.1f}}%<extra></extra>",
    ))
    fig.update_layout(
        barmode="stack", title=title, template="plotly_dark",
        height=390, margin=dict(t=50, b=20, l=0, r=10),
        xaxis_title="Oran (%)" if lang == "TR" else "Rate (%)",
        yaxis_title="Dil" if lang == "TR" else "Language",
        legend=dict(orientation="h", yanchor="top", y=1.12, x=0),
    )
    return fig


def render_bolgesel_analiz(lang: str):
    if lang == "TR":
        st.markdown("## 🌍 Bölgesel Analiz")
        st.caption("Farklı dil topluluklarının şikayet örüntülerini karşılaştırın")
        oyun_lbl      = "Oyun Seçin"
        sol_lbl       = "📊 Dil Bazlı Şikayet Dağılımı"
        ort_lbl       = "😊 Duygu Karşılaştırması"
        sag_lbl       = "📝 Bölgesel Bulgular"
        en_memnun_lbl = "En Memnun Topluluk"
        en_sik_lbl    = "En Çok Şikayet Eden"
        en_buyuk_lbl  = "En Büyük Topluluk"
        no_data       = "Yeterli veri bulunamadı."
    else:
        st.markdown("## 🌍 Regional Analysis")
        st.caption("Compare complaint patterns across different language communities")
        oyun_lbl      = "Select Game"
        sol_lbl       = "📊 Complaint Distribution by Language"
        ort_lbl       = "😊 Sentiment Comparison"
        sag_lbl       = "📝 Regional Insights"
        en_memnun_lbl = "Most Satisfied Community"
        en_sik_lbl    = "Most Complaining"
        en_buyuk_lbl  = "Largest Community"
        no_data       = "Not enough data available."

    tum_oyunlar = [(o, OYUN_ADLARI.get(o, o)) for tur in TURLER.values() for o in tur]
    oyun_idx    = st.selectbox(
        oyun_lbl, range(len(tum_oyunlar)),
        format_func=lambda i: tum_oyunlar[i][1], key="bolge_oyun",
    )
    secili_oyun = tum_oyunlar[oyun_idx][0]
    ...
    try:
        ...
        dil_df = _dil_metrikleri(df)
        if dil_df.empty or len(dil_df) < 2:
            st.warning(no_data)
            return

        # ── İki grafik + metin bölümü ──────────────────────────────────────
        col_s, col_o = st.columns([1.3, 1.1])
        with col_s:
            st.markdown(f"<div class='sec-title'>{sol_lbl}</div>", unsafe_allow_html=True)
            st.plotly_chart(_grafik_dil_sikayet(dil_df, lang), use_container_width=True)
        with col_o:
            st.markdown(f"<div class='sec-title'>{ort_lbl}</div>", unsafe_allow_html=True)
            st.plotly_chart(_grafik_dil_duygu(dil_df, lang), use_container_width=True)

        # ── Bölgesel bulgular (tam genişlik) ───────────────────────────────
        st.markdown(f"<div class='sec-title'>{sag_lbl}</div>", unsafe_allow_html=True)

        etiketler = kat_etiketler(lang)
        kat_keys  = [k for k in _KAT_TR.keys() if k in dil_df.columns]

        en_memnun_idx = dil_df["pos_pct"].idxmax()
        en_memnun_dil = dil_df.at[en_memnun_idx, "dil_ad"]
        en_memnun_pct = dil_df.at[en_memnun_idx, "pos_pct"]

        en_sik_idx = dil_df["neg_pct"].idxmax()
        en_sik_dil = dil_df.at[en_sik_idx, "dil_ad"]
        en_sik_pct = dil_df.at[en_sik_idx, "neg_pct"]

        en_buyuk_idx = dil_df["toplam"].idxmax()
        en_buyuk_dil = dil_df.at[en_buyuk_idx, "dil_ad"]
        en_buyuk_n   = dil_df.at[en_buyuk_idx, "toplam"]

        genel_neg    = dil_df["neg_pct"].mean()
        genel_en_kat = max(kat_keys, key=lambda k: dil_df[k].mean()) if kat_keys else ""
        genel_en_lab = etiketler.get(genel_en_kat, genel_en_kat)

        m1, m2, m3 = st.columns(3)
        if lang == "TR":
            m1.metric(en_memnun_lbl, en_memnun_dil, f"%{en_memnun_pct:.1f} olumlu")
            m2.metric(en_sik_lbl,    en_sik_dil,    f"%{en_sik_pct:.1f} olumsuz")
            m3.metric(en_buyuk_lbl,  en_buyuk_dil,  f"{en_buyuk_n:,} yorum")
        else:
            m1.metric(en_memnun_lbl, en_memnun_dil, f"{en_memnun_pct:.1f}% positive")
            m2.metric(en_sik_lbl,    en_sik_dil,    f"{en_sik_pct:.1f}% negative")
            m3.metric(en_buyuk_lbl,  en_buyuk_dil,  f"{en_buyuk_n:,} reviews")

        # Dil bazlı en yüksek şikayet kategorisi listesi
        st.markdown("")
        kart_cols = st.columns(min(4, len(dil_df)))
        for ci, (_, row) in enumerate(dil_df.iterrows()):
            if kat_keys:
                en_k = max(kat_keys, key=lambda k: row.get(k, 0))
                en_v = row.get(en_k, 0)
                en_l = etiketler.get(en_k, en_k)
            else:
                en_l, en_v = "-", 0
            with kart_cols[ci % len(kart_cols)]:
                pct_str = f"%{en_v:.1f}" if lang == "TR" else f"{en_v:.1f}%"
                st.markdown(
                    f'<div style="background:#1e293b;border-top:3px solid #3b82f6;'
                    f'border-radius:8px;padding:0.7rem;text-align:center;margin-bottom:0.4rem;">'
                    f'<div style="font-size:0.88rem;font-weight:600;color:#f1f5f9;">'
                    f'{row["dil_ad"]}</div>'
                    f'<div style="font-size:0.78rem;color:#94a3b8;margin-top:0.2rem;">'
                    f'{en_l}</div>'
                    f'<div style="font-size:1rem;font-weight:700;color:#f87171;">'
                    f'{pct_str}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )

        # Otomatik içgörü metni
        oran = en_sik_pct / max(0.01, genel_neg)
        if lang == "TR":
            st.info(
                f"**{en_sik_dil}** bu oyunu **%{en_sik_pct:.1f}** oranında olumsuz değerlendiriyor, "
                f"bu genel ortalamanın (**%{genel_neg:.1f}**) **{oran:.1f}x** katı. "
                f"En memnun topluluk **{en_memnun_dil}** (**%{en_memnun_pct:.1f}** olumlu). "
                f"Tüm dil toplulukları arasında en yaygın şikayet kategorisi: **{genel_en_lab}**."
            )
        else:
            st.info(
                f"**{en_sik_dil}** rates this game negatively at **{en_sik_pct:.1f}%**, "
                f"which is **{oran:.1f}x** the overall average (**{genel_neg:.1f}%**). "
                f"The most satisfied community is **{en_memnun_dil}** "
                f"(**{en_memnun_pct:.1f}%** positive). "
                f"Most common complaint category across all language groups: **{genel_en_lab}**."
            )
    except Exception:
        st.warning(no_data)

    st.divider()
    _render_pdf_butonu(secili_oyun, lang, "bolge")
...
# ─── MODÜL 8: Oyuncu Segmentasyonu ───────────────────────────────────────────

_SEGMENTLER = {
    "sadik_elestirmen": {
        "isim": "Sadık Eleştirmen", "renk": "#ef4444", "ikon": "🔴",
        "aciklama": "Çok oynadı, memnun değil",
    },
    "memnun_tutucu": {
        "isim": "Memnun Tutucu",   "renk": "#22c55e", "ikon": "🟢",
        "aciklama": "Çok oynadı ve memnun",
    },
    "hayal_kirikligi": {
        "isim": "Hayal Kırıklığı", "renk": "#f97316", "ikon": "🟠",
        "aciklama": "Az oynadı, terk etti",
    },
    "anlik_memnun": {
        "isim": "Anlık Memnun",    "renk": "#3b82f6", "ikon": "🔵",
        "aciklama": "Az oynadı ama memnun",
    },
}


def _segment_ata(df: pd.DataFrame) -> pd.DataFrame:
    df = df.copy()
    if "playtime_forever" in df.columns:
        pt = pd.to_numeric(df["playtime_forever"], errors="coerce").fillna(0)
    else:
        pt = pd.Series(0.0, index=df.index)
    voted = df["voted_up"].astype(bool) if "voted_up" in df.columns else pd.Series(True, index=df.index)
    conds = [
        (pt >= 600) & (~voted),
        (pt >= 600) & (voted),
        (pt < 60)   & (~voted),
    ...
    segs = ["sadik_elestirmen", "memnun_tutucu", "hayal_kirikligi", "anlik_memnun"]
    df["_segment"] = np.select(conds, segs, default="diger")
    df["_pt"] = pt
    return df


def render_oyuncu_segmentleri(lang: str):
    st.markdown("## 👥 Oyuncu Segmentleri")
    st.caption("Oyuncuları deneyim ve memnuniyet düzeyine göre 4 segmente ayırarak analiz edin")
    no_data   = "Yeterli veri bulunamadı."
    etiketler = kat_etiketler(lang)
    kat_keys  = list(_KAT_TR.keys())
    tum_oyunlar = [(o, OYUN_ADLARI.get(o, o)) for tur in TURLER.values() for o in tur]
    ...
    oyun_idx = st.selectbox("Oyun Seçin", range(len(tum_oyunlar)),
                            format_func=lambda i: tum_oyunlar[i][1], key="segment_oyun")
    secili_oyun = tum_oyunlar[oyun_idx][0]
    ...
    try:
        ...
        # ── 4 segment kartı ───────────────────────────────────────────────
        st.markdown("<div class='sec-title'>🗂️ Segment Dağılımı</div>", unsafe_allow_html=True)

        segment_meta = {}
        kart_cols = st.columns(4)
        for ci, (seg_key, seg_info) in enumerate(_SEGMENTLER.items()):
            seg_df = df[df["_segment"] == seg_key]
            n   = len(seg_df)
            pct = n / max(1, toplam) * 100
            segment_meta[seg_key] = {"df": seg_df, "n": n, "pct": pct}

            if n > 0:
                kat_sayilari = {etiketler[k]: int(seg_df[k].sum())
                                for k in kat_keys if k in seg_df.columns}
                en_kat = max(kat_sayilari, key=kat_sayilari.get) if kat_sayilari and max(kat_sayilari.values()) > 0 else "—"
            else:
                en_kat = "—"

            with kart_cols[ci]:
                st.markdown(
                    f'<div style="background:#1e293b;border-top:4px solid {seg_info["renk"]};'
                    f'border-radius:10px;padding:1rem 0.8rem;text-align:center;">'
                    f'<div style="font-size:1.4rem;">{seg_info["ikon"]}</div>'
                    f'<div style="font-size:0.88rem;font-weight:700;color:#f1f5f9;margin:0.25rem 0;">'
                    f'{seg_info["isim"]}</div>'
                    f'<div style="font-size:0.74rem;color:#94a3b8;margin-bottom:0.4rem;">'
                    f'{seg_info["aciklama"]}</div>'
                    f'<div style="font-size:1.65rem;font-weight:800;color:{seg_info["renk"]};">'
                    f'{n:,}</div>'
                    f'<div style="font-size:0.76rem;color:#94a3b8;">%{pct:.1f} oyuncu</div>'
                    f'<div style="font-size:0.72rem;color:#64748b;margin-top:0.35rem;">'
                    f'Başlıca şikayet: {en_kat}</div>'
                    f'</div>',
                    unsafe_allow_html=True,
                )

        # ── Grouped bar chart ──────────────────────────────────────────────
        st.markdown("<div class='sec-title'>📊 Segment Bazlı Şikayet Karşılaştırması</div>", unsafe_allow_html=True)

        kat_labels = [etiketler[k] for k in kat_keys]
        fig_grp    = go.Figure()
        for seg_key, seg_info in _SEGMENTLER.items():
            seg_df = segment_meta[seg_key]["df"]
            if seg_df.empty:
                continue
            y_vals = [round(seg_df[k].mean() * 100, 1) if k in seg_df.columns else 0.0
                      for k in kat_keys]
            fig_grp.add_trace(go.Bar(
                name=seg_info["isim"],
                x=kat_labels, y=y_vals,
                marker_color=seg_info["renk"],
                hovertemplate=(
                    f"<b>%{{x}}</b><br>"
                    f"{seg_info['isim']}<br>"
                    f"Şikayet Oranı: %{{y:.1f}}%<extra></extra>"
                ),
            ))
        fig_grp.update_layout(
            barmode="group", template="plotly_dark", height=380,
            margin=dict(t=20, b=10, l=0, r=0),
            xaxis_title="Şikayet Kategorisi",
            yaxis_title="Şikayet Oranı (%)",
            legend=dict(orientation="h", yanchor="bottom", y=1.01, x=0),
        )
        st.plotly_chart(fig_grp, use_container_width=True)

        # ── Segment profil tablosu ─────────────────────────────────────────
        st.markdown("<div class='sec-title'>📋 Segment Profil Tablosu</div>", unsafe_allow_html=True)
        ...
        for seg_key, seg_info in _SEGMENTLER.items():
            ...
            if n == 0:
                profil_rows.append({
                    "Segment": f"{seg_info['ikon']} {seg_info['isim']}",
                    "Oyuncu": "0", "Ort. Süre (saat)": "—",
                    "Baskın Dil": "—", "En Fazla Şikayet": "—",
                })
                continue
            ort_saat = seg_df["_pt"].mean() / 60
            if dil_col and dil_col in seg_df.columns:
                baskın = seg_df[dil_col].value_counts().index[0]
                baskın_dil = _DIL_GORUNUM.get(str(baskın).lower(), str(baskın).capitalize())
            else:
                baskın_dil = "—"
            kat_say = {etiketler[k]: int(seg_df[k].sum()) for k in kat_keys if k in seg_df.columns}
            en_kat  = max(kat_say, key=kat_say.get) if kat_say and max(kat_say.values()) > 0 else "—"
            profil_rows.append({
                "Segment":          f"{seg_info['ikon']} {seg_info['isim']}",
                "Oyuncu":           f"{n:,}",
                "Ort. Süre (saat)": f"{ort_saat:.0f}",
                "Baskın Dil":       baskın_dil,
                "En Fazla Şikayet": en_kat,
            })

        if profil_rows:
            st.dataframe(pd.DataFrame(profil_rows), use_container_width=True, hide_index=True)

        # ── Otomatik analiz metni ──────────────────────────────────────────
        st.markdown("<div class='sec-title'>📝 Otomatik Segment Analizi</div>", unsafe_allow_html=True)

        sadik  = segment_meta["sadik_elestirmen"]
        hayal  = segment_meta["hayal_kirikligi"]
        memnun = segment_meta["memnun_tutucu"]

        if sadik["n"] > 0:
            sadik_kat = {etiketler[k]: int(sadik["df"][k].sum())
                         for k in kat_keys if k in sadik["df"].columns}
            sadik_en_kat = max(sadik_kat, key=sadik_kat.get) if sadik_kat and max(sadik_kat.values()) > 0 else "-"
            sadik_en_pct = sadik_kat.get(sadik_en_kat, 0) / max(1, sadik["n"]) * 100
        else:
            sadik_en_kat, sadik_en_pct = "-", 0.0

        st.info(
            f"- **Sadık Eleştirmenlerin** %{sadik_en_pct:.1f}'i **{sadik_en_kat}** kategorisini "
            f"şikayet ediyor — bu sorun çözülmezse en değerli oyuncular kaybedilebilir.\n"
            f"- **Hayal Kırıklığı** segmenti toplam oyuncuların **%{hayal['pct']:.1f}'ini** oluşturuyor "
            f"— ilk izlenim iyileştirilmesi gerekmektedir.\n"
            f"- **Memnun Tutucu** oyuncular toplam oyuncuların **%{memnun['pct']:.1f}'ini** oluşturuyor "
            f"— **{oyun_ad}**'in güçlü bir oyuncu tabanı bulunmaktadır."
        )
    except Exception:
        st.warning(no_data)

    st.divider()
    _render_pdf_butonu(secili_oyun, lang, "segment")


# ─── MODÜL 9: Güncelleme Etkisi Analizi ──────────────────────────────────────

def render_guncelleme_etkisi(lang: str):
    st.markdown("## 🔄 Güncelleme Etkisi Analizi")
    st.caption("Belirli bir güncelleme tarihinin öncesi/sonrası oyuncu memnuniyetine etkisini ölçün")
    no_data   = "Yeterli veri bulunamadı."
    etiketler = kat_etiketler(lang)
    kat_keys  = list(_KAT_TR.keys())
    tum_oyunlar = [(o, OYUN_ADLARI.get(o, o)) for tur in TURLER.values() for o in tur]
    ...
    with c1:
        oyun_idx    = st.selectbox("Oyun Seçin", range(len(tum_oyunlar)),
                                   format_func=lambda i: tum_oyunlar[i][1], key="guncelleme_oyun")
        secili_oyun = tum_oyunlar[oyun_idx][0]
    with c2:
        varsayilan  = (pd.Timestamp.now() - pd.Timedelta(days=365)).date()
        update_date = st.date_input("Güncelleme Tarihi", value=varsayilan, key="guncelleme_tarihi")
    with c3:
        pencere_map = {"1 Ay": 30, "3 Ay": 90, "6 Ay": 180}
        pencere_lbl = st.selectbox("Karşılaştırma Penceresi", list(pencere_map.keys()), key="guncelleme_pencere")
        pencere_gun = pencere_map[pencere_lbl]
    ...
    try:
        ...
        df = df.copy()
        df["_ts"] = _parse_timestamp(df)
        df = df.dropna(subset=["_ts"])
        if df.empty:
            st.warning(no_data)
            return

        update_dt       = pd.Timestamp(update_date)
        onceki_baslangic = update_dt - pd.Timedelta(days=pencere_gun)
        sonraki_bitis   = update_dt + pd.Timedelta(days=pencere_gun)

        once_mask  = (df["_ts"] >= onceki_baslangic) & (df["_ts"] <  update_dt)
        sonra_mask = (df["_ts"] >= update_dt)         & (df["_ts"] <= sonraki_bitis)
        once_df    = df[once_mask]
        sonra_df   = df[sonra_mask]

        if len(once_df) < 3 and len(sonra_df) < 3:
            st.warning(
                f"Seçilen pencerede ({pencere_lbl}) yeterli veri bulunamadı. "
                f"Farklı bir tarih veya daha uzun bir pencere deneyin."
            )
            return

        oyun_ad = OYUN_ADLARI.get(secili_oyun, secili_oyun)

        # ── Genel memnuniyet değişimi ──────────────────────────────────────
        st.markdown("<div class='sec-title'>📈 Genel Memnuniyet Değişimi</div>", unsafe_allow_html=True)

        def _pos_pct(d):
            return float(d["voted_up"].astype(bool).mean() * 100) if "voted_up" in d.columns and len(d) > 0 else 0.0

        once_pos  = _pos_pct(once_df)
        sonra_pos = _pos_pct(sonra_df)
        degisim   = sonra_pos - once_pos

        m1, m2, m3, m4 = st.columns(4)
        m1.metric("Güncelleme Öncesi", f"%{once_pos:.1f}", f"{len(once_df):,} yorum")
        m2.metric("Güncelleme Sonrası", f"%{sonra_pos:.1f}", f"{len(sonra_df):,} yorum")
        m3.metric("Memnuniyet Değişimi", f"{degisim:+.1f}%",
                  "artış ✓" if degisim >= 0 else "düşüş ✗")
        m4.metric("Analiz Edilen Yorum", f"{len(once_df)+len(sonra_df):,}", f"{pencere_lbl} × 2")

        if degisim >= 10:
            st.success(
                f"✅ **Başarılı Güncelleme:** Bu güncelleme **{oyun_ad}**'in oyuncu memnuniyetini "
                f"önemli ölçüde artırdı (Δ{degisim:+.1f}%)."
            )
        elif degisim >= 0:
            st.warning(
                f"⚠️ **Kısmi Etki:** Güncelleme sınırlı iyileşme sağladı (Δ{degisim:+.1f}%). "
                f"İyileştirme sürdürülmeli."
            )
        else:
            st.error(
                f"❌ **Olumsuz Etki:** Bu güncelleme sonrası şikayetler arttı (Δ{degisim:+.1f}%). "
                f"Sorunların gözden geçirilmesi önerilir."
            )

        # ── Kategori önce/sonra karşılaştırması ───────────────────────────
        st.markdown("<div class='sec-title'>📊 Kategori Bazlı Önce/Sonra Karşılaştırması</div>", unsafe_allow_html=True)

        kat_labels_list, once_pcts, sonra_pcts = [], [], []
        for k in kat_keys:
            if k not in df.columns:
                continue
            o_p = float(once_df[k].mean()  * 100) if len(once_df)  > 0 else 0.0
            s_p = float(sonra_df[k].mean() * 100) if len(sonra_df) > 0 else 0.0
            kat_labels_list.append(etiketler[k])
            once_pcts.append(round(o_p, 1))
            sonra_pcts.append(round(s_p, 1))

        if kat_labels_list:
            fig_karsi = go.Figure()
            fig_karsi.add_trace(go.Bar(
                name=f"Öncesi ({pencere_lbl})", x=kat_labels_list, y=once_pcts,
                marker_color="#3b82f6",
                hovertemplate="<b>%{x}</b><br>Güncelleme Öncesi: %{y:.1f}%<extra></extra>",
            ))
            fig_karsi.add_trace(go.Bar(
                name=f"Sonrası ({pencere_lbl})", x=kat_labels_list, y=sonra_pcts,
                marker_color="#f97316",
                hovertemplate="<b>%{x}</b><br>Güncelleme Sonrası: %{y:.1f}%<extra></extra>",
            ))
            for i, (kat, dg) in enumerate(zip(kat_labels_list, [s - o for o, s in zip(once_pcts, sonra_pcts)])):
                if abs(dg) > 0.5:
                    renk = "#4ade80" if dg < 0 else "#f87171"
                    fig_karsi.add_annotation(
                        x=kat,
                        y=max(once_pcts[i], sonra_pcts[i]) + 1.5,
                        text=f"{dg:+.1f}%", showarrow=False,
                        font=dict(color=renk, size=10),
                    )
            fig_karsi.update_layout(
            ...
            st.plotly_chart(fig_karsi, use_container_width=True)

        # ── Aylık trend çizgisi ────────────────────────────────────────────
        if "voted_up" in df.columns:
            st.markdown("<div class='sec-title'>📉 Aylık Olumlu Yorum Trendi</div>", unsafe_allow_html=True)

            df2 = df.copy()
            df2["_ay"] = df2["_ts"].dt.to_period("M")
            aylik = (df2.groupby("_ay")
                     .agg(toplam=("voted_up", "count"), olumlu=("voted_up", "sum"))
                     .reset_index())
            aylik["pos_pct"] = aylik["olumlu"] / aylik["toplam"].clip(lower=1) * 100
            aylik["ay_str"]  = aylik["_ay"].astype(str)
            aylik = aylik.sort_values("_ay").reset_index(drop=True)

            if len(aylik) >= 2:
                fig_trend = go.Figure()
                fig_trend.add_trace(go.Scatter(
                    x=aylik["ay_str"], y=aylik["pos_pct"].round(1),
                    mode="lines+markers", name="Olumlu Yorum %",
                    line=dict(color="#3b82f6", width=2.5), marker=dict(size=6),
                    hovertemplate="<b>%{x}</b><br>Olumlu Oran: %{y:.1f}%<extra></extra>",
                ))
                update_ay_str = update_dt.to_period("M").strftime("%Y-%m")
                if update_ay_str in aylik["ay_str"].values:
                    fig_trend.add_vline(
                        x=update_ay_str,
                        line_width=2, line_dash="dash", line_color="#f59e0b",
                        annotation_text=" Güncelleme",
                ...
                fig_trend.update_layout(
                    ...
                    xaxis_title="Ay",
                    yaxis_title="Olumlu Yorum Oranı (%)",
                    showlegend=False,
                )
    ...
    st.divider()
    _render_pdf_butonu(secili_oyun, lang, "guncelleme")
...
def _pdf_rapor_olustur(oyun_key: str, lang: str) -> bytes:
    ...
    # ── Sayfa 4: Oyuncu Segmentleri ──────────────────────────────────────
    story.append(_H2("Oyuncu Segmentleri"))
    df_seg_pdf = _segment_ata(df) if not df.empty else pd.DataFrame()
    seg_data_pdf = [["Segment", "Oyuncu", "Oran %", "Ort. Süre (saat)", "Baş. Şikayet"]]
    for seg_key, seg_info in _SEGMENTLER.items():
        seg_s = (df_seg_pdf[df_seg_pdf["_segment"] == seg_key]
                 if "_segment" in df_seg_pdf.columns else pd.DataFrame())
        n_s   = len(seg_s)
        pct_s = n_s / max(1, toplam) * 100
        ort_s = seg_s["_pt"].mean() / 60 if n_s > 0 and "_pt" in seg_s.columns else 0.0
        ks    = {_KAT_TR[k]: int(seg_s[k].sum()) for k in _KK if k in seg_s.columns} if n_s > 0 else {}
        ek    = max(ks, key=ks.get) if ks and max(ks.values()) > 0 else "-"
        seg_data_pdf.append([seg_info["isim"], str(n_s), f"{pct_s:.1f}%", f"{ort_s:.0f}", ek])
    story.append(_tbl(seg_data_pdf, col_w=[4 * _cm, 2.5 * _cm, 2.5 * _cm, 4 * _cm, 4 * _cm]))
    story.append(_PB())

    # ── Sayfa 5: Aksiyon Önerileri ────────────────────────────────────────
...
_AY_ADLARI_EN = ["Jan", "Feb", "Mar", "Apr", "May", "Jun",
                  "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
_GUN_ADLARI_TR = ["Pazartesi", "Salı", "Çarşamba", "Perşembe", "Cuma", "Cumartesi", "Pazar"]
_GUN_ADLARI_EN = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"]


def render_mevsimsellik(lang: str):
    st.markdown("## 📅 Mevsimsellik Analizi")
    st.caption("Şikayetlerin hangi dönemlerde yoğunlaştığını ve mevsimsel örüntüleri keşfedin"
               if lang == "TR" else
               "Discover seasonal patterns and peak periods in player complaints")
    no_data   = "Yeterli veri bulunamadı." if lang == "TR" else "Not enough data available."
    ay_adlari = _AY_ADLARI_TR if lang == "TR" else _AY_ADLARI_EN
    gun_adlari= _GUN_ADLARI_TR if lang == "TR" else _GUN_ADLARI_EN
    etiketler = kat_etiketler(lang)
    kat_keys  = list(_KAT_TR.keys())
    tum_oyunlar = [(o, OYUN_ADLARI.get(o, o)) for tur in TURLER.values() for o in tur]

    oyun_lbl_m = "Oyun Seçin" if lang == "TR" else "Select Game"
    oyun_idx = st.selectbox(oyun_lbl_m, range(len(tum_oyunlar)),
                             format_func=lambda i: tum_oyunlar[i][1], key="mevs_oyun")
    secili_oyun = tum_oyunlar[oyun_idx][0]
    ...
    try:
        ...
        df["_ay_num"]  = df["_ts"].dt.month
        df["_gun_num"] = df["_ts"].dt.dayofweek
        df["_yilAy"]   = df["_ts"].dt.to_period("M")
        oyun_ad = OYUN_ADLARI.get(secili_oyun, secili_oyun)

        # ── 1. Aylık yoğunluk ısı haritası ──────────────────────────────
        if lang == "TR":
            st.markdown("<div class='sec-title'>🗓️ Aylık Şikayet Yoğunluğu Isı Haritası</div>",
                        unsafe_allow_html=True)
        else:
            st.markdown("<div class='sec-title'>🗓️ Monthly Complaint Intensity Heatmap</div>",
                        unsafe_allow_html=True)
        ...
        for k in kat_keys:
            ...
            for ay in range(1, 13):
                val = int(df[df["_ay_num"] == ay][k].sum()) if k in df.columns else 0
                row_val.append(val)
                row_hov.append(
                    f"{ay_adlari[ay-1]} — {etiketler.get(k, k)}: {val} yorum" if lang == "TR"
                    else f"{ay_adlari[ay-1]} — {etiketler.get(k, k)}: {val} reviews"
                )
            heat_matrix.append(row_val)
            hover_texts.append(row_hov)

        fig_heat = go.Figure(go.Heatmap(
            z=heat_matrix,
            x=ay_adlari,
            y=[etiketler[k] for k in kat_keys],
            colorscale=[[0, "#fefce8"], [0.33, "#fde68a"], [0.66, "#f97316"], [1, "#991b1b"]],
            text=hover_texts,
            hovertemplate="%{text}<extra></extra>",
            showscale=True,
            colorbar=dict(title="Yorum Sayısı" if lang == "TR" else "Reviews"),
        ))
        fig_heat.update_layout(
            template="plotly_dark", height=380,
            margin=dict(t=20, b=20, l=0, r=0),
            xaxis_title="Ay" if lang == "TR" else "Month",
            yaxis_title="Şikayet Kategorisi" if lang == "TR" else "Complaint Category",
        )
        st.plotly_chart(fig_heat, use_container_width=True)

        # ── 2. Haftalık dağılım ───────────────────────────────────────────
        if lang == "TR":
            st.markdown("<div class='sec-title'>📅 Haftalık Şikayet Dağılımı</div>",
                        unsafe_allow_html=True)
        else:
            st.markdown("<div class='sec-title'>📅 Weekly Complaint Distribution</div>",
                        unsafe_allow_html=True)

        gun_sayilari = df.groupby("_gun_num").size().reindex(range(7), fill_value=0)
        max_gun_idx  = int(gun_sayilari.argmax())
        max_gun_ad   = gun_adlari[max_gun_idx]
        fig_gun = go.Figure(go.Bar(
            x=gun_adlari,
            y=gun_sayilari.values,
            marker_color=["#ef4444" if i == max_gun_idx else "#3b82f6" for i in range(7)],
            hovertemplate=(
                "<b>%{x}</b><br>Yorum Sayısı: %{y:,}<extra></extra>" if lang == "TR"
                else "<b>%{x}</b><br>Reviews: %{y:,}<extra></extra>"
            ),
        ))
        baslik_gun = (f"Oyuncular en çok <b>{max_gun_ad}</b> günü şikayet ediyor" if lang == "TR"
                      else f"Players complain most on <b>{max_gun_ad}</b>")
        fig_gun.update_layout(
            template="plotly_dark", height=300,
            margin=dict(t=40, b=10, l=0, r=0),
            title=dict(text=baslik_gun, font=dict(size=13)),
            xaxis_title="Haftanın Günü" if lang == "TR" else "Day of Week",
            yaxis_title="Yorum Sayısı" if lang == "TR" else "Review Count",
        )
        st.plotly_chart(fig_gun, use_container_width=True)

        # ── 3. Yıllık trend çizgisi ───────────────────────────────────────
        if lang == "TR":
            st.markdown("<div class='sec-title'>📈 Yıllık Trend — Olumlu & Olumsuz</div>",
                        unsafe_allow_html=True)
        else:
            st.markdown("<div class='sec-title'>📈 Annual Trend — Positive & Negative</div>",
                        unsafe_allow_html=True)

        if "voted_up" in df.columns:
            aylik_tr = (df.groupby("_yilAy")
                        .agg(toplam=("voted_up", "count"), olumlu=("voted_up", "sum"))
                        .reset_index())
            aylik_tr["olumsuz"] = aylik_tr["toplam"] - aylik_tr["olumlu"]
            aylik_tr["ay_str"]  = aylik_tr["_yilAy"].astype(str)
            aylik_tr = aylik_tr.sort_values("_yilAy").reset_index(drop=True)

            if len(aylik_tr) >= 2:
                fig_tr = go.Figure()
                fig_tr.add_trace(go.Scatter(
                    x=aylik_tr["ay_str"], y=aylik_tr["olumlu"],
                    mode="lines+markers",
                    name="Olumlu" if lang == "TR" else "Positive",
                    line=dict(color="#22c55e", width=2.2), marker=dict(size=5),
                    hovertemplate="<b>%{x}</b><br>Olumlu: %{y:,}<extra></extra>" if lang == "TR"
                                  else "<b>%{x}</b><br>Positive: %{y:,}<extra></extra>",
                ))
                fig_tr.add_trace(go.Scatter(
                    x=aylik_tr["ay_str"], y=aylik_tr["olumsuz"],
                    mode="lines+markers",
                    name="Olumsuz" if lang == "TR" else "Negative",
                    line=dict(color="#ef4444", width=2.2), marker=dict(size=5),
                    hovertemplate="<b>%{x}</b><br>Olumsuz: %{y:,}<extra></extra>" if lang == "TR"
                                  else "<b>%{x}</b><br>Negative: %{y:,}<extra></extra>",
                ))
                max_neg_idx = aylik_tr["olumsuz"].idxmax()
                max_neg_ay  = aylik_tr.at[max_neg_idx, "ay_str"]
                max_neg_val = aylik_tr.at[max_neg_idx, "olumsuz"]
                fig_tr.add_annotation(
                    x=max_neg_ay, y=max_neg_val,
                    text="▲ En Yüksek" if lang == "TR" else "▲ Peak",
                    showarrow=True, arrowhead=2,
                    font=dict(color="#ef4444", size=11), arrowcolor="#ef4444",
                )
        ...
        # ── 4. Büyük artış (lansmanı etkisi) ─────────────────────────────
        if lang == "TR":
            st.markdown("<div class='sec-title'>🚀 Büyük Artış / Lansmanı Etkisi</div>",
                        unsafe_allow_html=True)
        else:
            st.markdown("<div class='sec-title'>🚀 Review Spike Detection</div>",
                        unsafe_allow_html=True)

        aylik_say = (df.groupby("_yilAy").size()
                     .reset_index(name="count").sort_values("_yilAy").reset_index(drop=True))
        aylik_say["ay_str"]    = aylik_say["_yilAy"].astype(str)
        aylik_say["change_pct"] = aylik_say["count"].pct_change() * 100
        spikes = aylik_say[aylik_say["change_pct"] >= 50].dropna(subset=["change_pct"])

        if not spikes.empty:
            st.warning(
                "Bu dönemlerde yorum sayısında anormal artış tespit edildi:" if lang == "TR"
                else "Abnormal review spikes detected:"
            )
            for _, rsp in spikes.iterrows():
                if lang == "TR":
                    st.markdown(f"- 📈 **{rsp['ay_str']}**: {int(rsp['count']):,} yorum — "
                                f"önceki aya göre **+{rsp['change_pct']:.0f}%** artış")
                else:
                    st.markdown(f"- 📈 **{rsp['ay_str']}**: {int(rsp['count']):,} reviews — "
                                f"**+{rsp['change_pct']:.0f}%** vs previous month")
        else:
            st.success("✅ " + ("Yorum sayısında anormal artış tespit edilmedi." if lang == "TR"
                                 else "No abnormal spikes detected."))

        # ── 5. Otomatik mevsimsellik içgörüsü ────────────────────────────
        if lang == "TR":
            st.markdown("<div class='sec-title'>📝 Otomatik Mevsimsellik İçgörüsü</div>",
                        unsafe_allow_html=True)
        else:
            st.markdown("<div class='sec-title'>📝 Auto Seasonality Insight</div>",
                        unsafe_allow_html=True)

        ay_toplam    = df.groupby("_ay_num").size()
        en_yogun_ay  = int(ay_toplam.idxmax())
        en_sakin_ay  = int(ay_toplam.idxmin())
        hafta_ici    = int(df[df["_gun_num"] < 5].shape[0])
        hafta_sonu   = int(df[df["_gun_num"] >= 5].shape[0])
        yaz_df_m     = df[df["_ay_num"].isin([6, 7, 8])]
        kis_df_m     = df[df["_ay_num"].isin([12, 1, 2])]
        yaz_kis_d    = (len(yaz_df_m) - len(kis_df_m)) / max(1, len(kis_df_m)) * 100

        if lang == "TR":
            hf_karsi = "hafta içi" if hafta_ici > hafta_sonu else "hafta sonu"
            yaz_yorum = (f"Yaz döneminde aktivite kış dönemine göre **{abs(yaz_kis_d):.0f}% "
                         f"{'artıyor' if yaz_kis_d > 0 else 'azalıyor'}**.")
            st.info(
                f"- **{oyun_ad}** en çok **{_AY_ADLARI_TR[en_yogun_ay - 1]}** ayında şikayet alıyor "
                f"({ay_toplam.get(en_yogun_ay, 0):,} yorum).\n"
                f"- Oyuncular daha fazla **{hf_karsi}** şikayet yapıyor "
                f"(hafta içi: {hafta_ici:,}, hafta sonu: {hafta_sonu:,}).\n"
                f"- {yaz_yorum}\n"
                f"- En sakin dönem: **{_AY_ADLARI_TR[en_sakin_ay - 1]}** — güncelleme için ideal zaman."
            )
        else:
            hf_karsi = "weekdays" if hafta_ici > hafta_sonu else "weekends"
            yaz_yorum = (f"Summer activity is **{abs(yaz_kis_d):.0f}% "
                         f"{'higher' if yaz_kis_d > 0 else 'lower'}** than winter.")
            st.info(
                f"- **{oyun_ad}** receives the most complaints in **{_AY_ADLARI_EN[en_yogun_ay - 1]}** "
                f"({ay_toplam.get(en_yogun_ay, 0):,} reviews).\n"
                f"- Players complain more on **{hf_karsi}** "
                f"(weekdays: {hafta_ici:,}, weekends: {hafta_sonu:,}).\n"
                f"- {yaz_yorum}\n"
                f"- Quietest month: **{_AY_ADLARI_EN[en_sakin_ay - 1]}** — ideal for updates."
            )

        st.divider()
        _render_pdf_butonu(secili_oyun, lang, "mevs")
